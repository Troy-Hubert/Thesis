...
```{r}
knitr::kable(Ins_Mod[1:5,], format="html")
```
...
knitr::kable(phc_IM, format="html")
knitr::kable(mhc_IM, format="html")
```
...
Repeat for Univaraite analysis
```{r,include=FALSE}
#### INS_MOD
#create the variable within the dataframe
...
Ins_Mod_Uni[23,9] = 24
# create a varaible for significance 
Ins_Mod_Uni <- mutate(Ins_Mod_Uni, sig = 0)
Ins_Mod_Uni[1,10]  = 1
Ins_Mod_Uni[3,10]  = 1
Ins_Mod_Uni[5,10]  = 1
Ins_Mod_Uni[6,10]  = 1
...
Ins_Act_Uni[23,9] = 24
# create a varaible for significance 
Ins_Act_Uni <- mutate(Ins_Act_Uni, sig = 0)
Ins_Act_Uni[1,10]  = 1
Ins_Act_Uni[3,10]  = 1
Ins_Act_Uni[5,10]  = 1
Ins_Act_Uni[6,10]  = 1
...
Act_Mod_Uni[23,9] = 24
# create a varaible for significance 
Act_Mod_Uni <- mutate(Act_Mod_Uni, sig = 0)
Act_Mod_Uni[6,10]  = 1
Act_Mod_Uni[9,10] = 1
Act_Mod_Uni[10,10] = 1
Act_Mod_Uni
...
```{r}
# Here is an example of two of the plots. 
phc_IA_left
mhc_IA_left
```
...
two <- mhc_IA_left + mhc_IA_mid + mhc_IA_right + plot_layout(design = layout)
```
...
I struggled to figure out how to combine these two figures together without `ggplot()` getting angery. So I used the forbidden **_Illustrator_** to tidy up everything and combine the univariate and multivariate plots. The final plots are below with explinations of their results.
...
_Univariate._ 
Similarily to sufficiently active, veterans tend who have health conditions are less likely to report moderate physical activity compared to those who are insuficiently active.
...
```{r,fig.align='center'}
knitr::include_graphics("~/Desktop/Coding/R working directory/plots/Mod_Act_FP.png")
```
_Univariate._
