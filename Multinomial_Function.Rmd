...
## Varience explained by health conditions alone

```{r}
# Variance explained:
Uni_R_AD <- multinom(relevel(Ex_3cat,ref = "Insufficient")  ~ Any_Disability, data = thesis_NHRVS)
Uni_R_MS <- multinom(relevel(Ex_3cat,ref = "Insufficient")  ~ MS, data = thesis_NHRVS)
print(paste0("The varaince explained by physical health conditions for the univariate model ranges from ",
     round(r2(Uni_R_MS)[[1]],5)," to ", round(r2(Uni_R_AD)[[1]],5)
     ))

thesis_NHRVS$LT_NicotineDep
Uni_R_ND <- multinom(relevel(Ex_3cat,ref = "Insufficient")  ~ LT_NicotineDep, data = thesis_NHRVS)
Uni_R_PTSD <- multinom(relevel(Ex_3cat,ref = "Insufficient")  ~ LT_PTSD, data = thesis_NHRVS)
print(paste0("The varaince explained by mental health conditions for the univariate model ranges from ",
     round(r2(Uni_R_PTSD)[[1]],5)," to ", round(r2(Uni_R_ND)[[1]],5)
     ))
```
...
Cov_Reg2<-multinom(relevel(Ex_3cat,ref = "Moderate")  ~ Age + College + Income_60k_plus + YN_work + BMI, data = thesis_NHRVS)
...
cov_OR_tab <-  data.frame(cbind(
  ...
  IvA_ODDS = OR.CI_IvA,
  IvA_p = round(p_val1[2,2:6],4),
  IvM_ODDS = OR.CI_IvM,
  IvM_p = round(p_val1[1,2:6],4),
  AvM_ODDS = OR.CI_AvM, 
  AvM_p = round(p_val2[2,2:6],4)
))
#2. create the row for the R2
  paste("Block 1 R^2 =", round((R2_Cov_Reg[[1]]*100),3),"%")
  variable = paste0("Block 1 R^2 =", round((R2_Cov_Reg[[1]]*100),3),"%")
...
#3. Bind the stuff together
  ...
  # cov_OR_tab <- mutate(cov_OR_tab,
  #   IvA_ODDS = ifelse(IvA_p <= .05 & IvA_p > .01, paste(IvA_ODDS, "*"),
  #              ifelse(IvA_p <= .01 & IvA_p > .001, paste(IvA_ODDS, "**"),
  #              ifelse(IvA_p <= .001, paste(IvA_ODDS, "***"), IvA_ODDS))),
  #   IvM_ODDS = ifelse(IvM_p <= .05 & IvM_p > .01, paste(IvM_ODDS, "*"),
  #              ifelse(IvM_p <= .01 & IvM_p > .001, paste(IvM_ODDS, "**"),
  #              ifelse(IvM_p <= .001, paste(IvM_ODDS, "***"), IvM_ODDS))),
  #   AvM_ODDS = ifelse(round(AvM_p,2) <= .05 & AvM_p > .01, paste(AvM_ODDS, "*"),
  #              ifelse(round(AvM_p,2) <= .01 & AvM_p > .001, paste(AvM_ODDS, "**"),
  #              ifelse(AvM_p <= .001, paste(AvM_ODDS, "***"), AvM_ODDS)))
  # )
...
#1. Run the models
  ...
  Cov.MCS_Reg2<-multinom(relevel(Ex_3cat,ref = "Moderate")  ~ Age + College + Income_60k_plus + YN_work + BMI + MCS, data = thesis_NHRVS)
  ...
  Cov.PCS_Reg2<-multinom(relevel(Ex_3cat,ref = "Moderate")  ~ Age + College + Income_60k_plus + YN_work + BMI + PCS, data = thesis_NHRVS)
