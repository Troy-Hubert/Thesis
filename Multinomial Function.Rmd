...
#2. run model 1
  if(type == "PHC"){
  df <- multinom(relevel(Ex_3cat,ref = "Insufficient")  ~ condition + Age + College + Income_60k_plus + YN_work + BMI, data = Data)
  }
  if(type == "MHC"){ df <- multinom(relevel(Ex_3cat,ref = "Insufficient")  ~ condition + Age + College + Income_60k_plus + YN_work + BMI, data = Data)
  }
#3. Specify the reference group2
  Data$Ex_3cat_act <- relevel(Data$Ex_3cat, ref = "Active")
#4. run model 2
  if(type == "PHC"){
   df2 <- multinom(relevel(Ex_3cat,ref = "Active") ~ condition + Age + College + Income_60k_plus + YN_work + BMI, data = Data)
  }
  if(type == "MHC"){ 
    df2 <- multinom(relevel(Ex_3cat,ref = "Active") ~ condition + Age + College + Income_60k_plus + YN_work + BMI, data = Data)
  }
...
```
### R^2 Change
The following function allows us to look at the [R^2 change](https://easystats.github.io/performance/reference/r2.html) between the covariates and the IV within the multinomial model. As this is a multinomial model, the R^2 change is only observed for the model as a whole.
...
output<-  r2(df)[[1]] -  r2(multi_cov)[[1]]
return(output)
}
R2CHANGE <- r2_chan(thesis_NHRVS$Any_Disability)[[1]]
